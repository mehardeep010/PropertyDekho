...
//FEEDBACK: Catching Payment_Type from UI and saving to DB
router.post('/pay-security', async (req, res) => {
  const { Lease_ID, Amount, Payment_Type, Method } = req.body;
  const tid  = req.user.ref_id;
  ...
  try {
    ...
    if (lease.Lease_Status === 'Terminated') return res.status(400).json({ error: 'This lease has been terminated.' });
    if (lease.Lease_Status === 'Active') return res.status(400).json({ error: 'Security deposit already paid.' });
    if (lease.seconds_elapsed > 7200) return res.status(400).json({ error: 'Time limit expired! Lease cancelled.' });

    await conn.beginTransaction();
    
    // Record payment with specific type (Security Deposit, Monthly Rent, etc.)
    const today = new Date().toISOString().slice(0,10);
    await conn.query(
      "INSERT INTO PAYMENT (Payment_Date,Amount,Payment_Type,Method,Status,Lease_ID) VALUES(?,?,?,?,?,?)",
      [today, Amount, Payment_Type || 'Security_Deposit', Method, 'Success', Lease_ID]
    );
    
    // Activate lease and set property to Rented
    ...
    await conn.commit();
    res.json({ message: ' Payment successful! Your lease is now Active.' });
  } catch (e) { 
    await conn.rollback(); 
...
router.post('/inquiries', async (req, res) => {
  ...
  const tid = req.user.ref_id;
  try {
    const [[prop]] = await db.query('SELECT Agent_ID FROM PROPERTY WHERE Property_ID=?', [Property_ID]);
...
router.get('/dashboard', async (req, res) => {
  ...
  try {
    ...
    // Safely check for sales without crashing if the table is empty/missing
    let pending_sales = 0;
    try {
      const [saleRes] = await db.query("SELECT COUNT(*) AS pending_sales FROM SALE WHERE Buyer_Tenant_ID=? AND Sale_Status='Pending_Payment'", [tid]);
      if (saleRes && saleRes.length) pending_sales = saleRes[0].pending_sales;
    } catch(err) { /* Sale table might not exist, ignore safely */ }
...
router.get('/sales', async (req, res) => {
  ...
  try {
    ...
    res.json(rows);
  } catch (e) { res.json([]); } // Return empty array if SALE table missing
});
...
router.post('/pay-sale', async (req, res) => {
  ...
  try {
    ...
    if (sale.seconds_elapsed > 10800)    return res.status(400).json({ error: 'Time limit expired! Sale cancelled.' });
    ...
    await conn.commit();
    res.json({ message: ' Payment successful! The property is now officially Sold.' });
