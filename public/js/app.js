...
const colDefs = {
  properties: {
    cols: ['ID','Title','Type','Location','Price','AI Est.','Status','Owner','Agent','Actions'],
    row: d => [
      d.Property_ID, d.Title, d.Type, d.Location,
      '₹'+fmtN(d.Price), '₹'+fmtN(d.AI_Est_Price),
      badge(d.Status), d.Owner_Name, d.Agent_Name,
...
function openModal(page, data = null) {
  ...
  modalForm.innerHTML = def.fields.map(f => {
    ...
    if (f.type === 'select') {
      ...
      input = `<input type="${f.type}" name="${f.name}" value="${val}" ${f.required?'required':''} />`;
    }
