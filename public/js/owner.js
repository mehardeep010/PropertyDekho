...
function renderMyPropsTable(data) {
  renderTable('propsTable',
    ['#','Title','Type','Location','Price','AI Est.','Status','Agent','Inquiries','Actions'],
    data.map(p => [
      p.Property_ID, p.Title, p.Type, p.Location,
      '₹'+fmtN(p.Price), '₹'+fmtN(p.AI_Est_Price),
      badge(p.Status), p.Agent_Name, p.inquiry_count || 0,
      p.Status === 'Sold'
...
function selectAgent(agentId) {
  document.getElementById('propAgentId').value = agentId;
  document.getElementById('agentMapPanel').classList.add('hidden');
  toast('✓ Agent #' + agentId + ' selected');
}
...
async function editProp(id) {
  try {
    ...
    document.getElementById('propAgentId').value    = p.Agent_ID;
    document.getElementById('modal').classList.remove('hidden');
  } catch (e) { toast(e.message, true); }
}
...
async function submitProp(e) {
  ...
  const body = {
    ...
    Owner_ID: REF_ID,
    Agent_ID: document.getElementById('propAgentId').value,
  };
  try {
    if (editPropId) {
