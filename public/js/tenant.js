// ============================================================
//  Tenant Portal Logic (Updated with Prof's Feedback)
// ============================================================
'use strict';
  ...
  try {
    ...
    document.getElementById('modalBody').innerHTML = `
      <div class="detail-grid" style="grid-template-columns:repeat(2,1fr);">
        ...
        <div class="detail-item"><label>Agent</label><p>${p.Agent_Name}</p></div>
      </div>
      ...
      ${p.Status==='Available'||p.Status==='Pending'
        ? `<button class="form-submit" onclick="openInquiryModal(${p.Property_ID},'${p.Title.replace(/'/g,"\\'")}');closeModal()">💬 Send Inquiry</button>` : ''}`;
    ...
    <div class="detail-panel" style="margin-bottom:16px;" id="lease-panel-${l.Lease_ID}">
      ...
      ${isPending ? `
        ...
        <div style="background:rgba(52,211,153,.06);border:1px solid rgba(52,211,153,.2);border-radius:12px;padding:18px;">
          <div style="font-size:13px;font-weight:600;color:var(--success);margin-bottom:14px;">💳 Make Payment</div>
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:12px;">
            ...
            <div class="form-group">
              ...
              <select id="payType-${l.Lease_ID}" style="background:#111326;border:1px solid var(--border);color:var(--text);padding:10px 13px;border-radius:10px;font-size:13px;">
            ...
            <div class="form-group">
              <label>Method</label>
              <select id="payMethod-${l.Lease_ID}" style="background:#111326;border:1px solid var(--border);color:var(--text);padding:10px 13px;border-radius:10px;font-size:13px;">
...
// UPDATED: Now sends Payment_Type to the backend
async function submitSecurityPayment(leaseId) {
  const amount = document.getElementById(`payAmt-${leaseId}`).value;
  const type = document.getElementById(`payType-${leaseId}`).value;
  ...
  try {
    const r = await api('/tenant-portal/pay-security', 'POST', { 
      Lease_ID: leaseId, 
      Amount: parseFloat(amount), 
      Payment_Type: type, // Added field
      Method: method 
    });
...
// Payments - UPDATED: Now shows Payment Type in the table
async function loadPayments() {
  const data = await api('/tenant-portal/payments');
  if (!data) return;
  renderTable('paymentsTable',
    ['#','Property','Date','Type','Amount','Method','Status'],
    data.map(p => [
      p.Payment_ID, 
      p.Property_Title, 
      fmtDate(p.Payment_Date), 
      (p.Payment_Type || 'Monthly_Rent').replace('_', ' '), // Show Type nicely
      '₹'+fmtN(p.Amount), 
      p.Method, 
      badge(p.Status)
    ])
